 ...
 * 1. 회원가입("/enroll") - get enroll.jsp , post 회원가입DB다녀와서 첫화면(/) 이동 id 중복확인
 * 회원가입화면 - 회원가입(enroll post) 버튼
 * 2. 로그인("/login") - Get login.jsp로 forward,  Post 로그인확인DB다녀와서 session("lgnss")등록(이름,email,id)  완료후 첫화면(/) 이동
